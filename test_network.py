...
def run_test(num_tests=10):
    ...
    for i in range(num_tests):
        ...
        input_signal = max(pixel) / 255.0
        # Supervised feedback: route signal and apply feedback to neurons
        route_result = controller.route_signal(input_signal, goal=true_color, true_label=true_color)
        # Automated feedback: adjust weights/specialty based on correctness
        for cluster in clusters:
            for neuron in cluster.neurons:
                neuron_guess = neuron.get_preferred_color()
                correct_guess = (neuron_guess == true_color)
                # If neuron is winner and wrong, weaken weights and specialty
                if neuron.is_winner and not correct_guess:
                    for k in neuron.weight:
                        neuron.weight[k] *= 0.95
                    neuron.specialty -= 0.1
                # If neuron is not winner but would have been correct, strengthen weights and specialty
                if not neuron.is_winner and correct_guess:
                    for k in neuron.weight:
                        neuron.weight[k] *= 1.05
                    neuron.specialty += 0.1
        ...
        # Collect test result for logging
        test_results.append(f"Test {i+1}: Pixel={pixel} | True={true_color} | Predicted={predicted_color} | Activations={activations}\n  Controller: {route_result}")
    ...
    print(f"\nAccuracy: {correct}/{num_tests} correct ({correct/num_tests*100:.1f}%)")
    # Log accuracy for training_run.txt
    test_results.append(f"\nAccuracy: {correct}/{num_tests} correct ({correct/num_tests*100:.1f}%)")
    print("\n=== Hierarchical Activity Summary ===")
    monitor.print_human_summary()
    # Append test results to training_run.txt
    with open("training_logs/training_run.txt", "a") as f:
